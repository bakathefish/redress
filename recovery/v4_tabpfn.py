"""v4 build step 6b: the second challenger, TabPFN v2 (Hollmann et al. 2025, Nature 637:319),
a tabular foundation model that classifies in context. Eight contexts per outer fold, each
...
def make_clf(seed):
    # the open TabPFN v2 checkpoint (Prior-Labs/TabPFN-v2-clf, Prior Labs licence); the
    # package default points at gated 2.5+ weights that need a browser login
    ckpt = open(os.path.join(OUT, "tabpfn_v2_ckpt.txt")).read().strip()
    return TabPFNClassifier(model_path=ckpt, device="cpu", random_state=seed)
...
for R in REGIONS:
    ...
    json.dump(
        results, open(os.path.join(OUT, f"tabpfn_results{SUF}.json"), "w"), indent=1
    )
...
oof.to_parquet(os.path.join(OUT, f"tabpfn_oof_scores{SUF}.parquet"), index=False)
...
json.dump(results, open(os.path.join(OUT, f"tabpfn_results{SUF}.json"), "w"), indent=1)
print("TabPFN pooled:", json.dumps(results["pooled"]), flush=True)
