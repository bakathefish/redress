...
def fit_powerlaw(lam, f, e, lo, hi, mask=None):
    ...
    n = int(m.sum())
    if n < MIN_PIX:
        return dict(bad, n=n, reason="only %d usable pixels" % n)

    x, y, s = lam[m], f[m], e[m]
    span = float(np.log10(x.max()) - np.log10(x.min()))
    if span < MIN_SPAN_DEX:
        return dict(
            bad, n=n, span_dex=span, reason="wavelength span only %.3f dex" % span
        )
    ...
    if not np.isfinite(amp_snr) or amp_snr < MIN_CONTINUUM_SNR:  # guard 1
        return dict(
            ...
            amp_snr=amp_snr,
            snr_pix=snr_pix,
            reason="continuum not detected (amplitude S/N %.2f < %.1f)"
            % (amp_snr if np.isfinite(amp_snr) else -1, MIN_CONTINUUM_SNR),
        )
